...
class _SellerPageState extends State<SellerPage> with TickerProviderStateMixin {
  ...
  Future<void> _create() async {
    ...
    try {
      final res = await http.get(Uri.parse(
          "http://server.sanzyoffc.panelantirusuh.biz.id:10604/api/user/createAccount?key=${widget.keyToken}&newUser=$u&pass=$p&day=$d"));
  ...
  Future<void> _edit() async {
    ...
    try {
      final res = await http.get(Uri.parse(
          "http://server.sanzyoffc.panelantirusuh.biz.id:10604/api/user/editUser?key=${widget.keyToken}&username=$u&addDays=$d"));
      final data = jsonDecode(res.body);
      if (data['edited'] == true) {
