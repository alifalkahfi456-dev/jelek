...
class _BugSenderPageState extends State<BugSenderPage>
  ...
  bool get _canUsePublic => _isOwner || _isVip || _isAdmin;
  ...
  Future<void> _addPrivate() async {
    ...
    try {
      final res = await http.get(
        Uri.parse('$kBaseUrl/getPairing?key=${widget.sessionKey}&number=$num'),
  ...
  Future<void> _deletePrivate(String sessionName) async {
    ...
    try {
      final res = await http.get(
        Uri.parse('$kBaseUrl/deleteSender?key=${widget.sessionKey}&session=$sessionName'),
      ).timeout(const Duration(seconds: 10));
      final d = jsonDecode(res.body);
      if (d['valid'] == true) {
  ...
  Future<void> _deletePublic(String sessionName) async {
    ...
    try {
      final res = await http.get(
        Uri.parse('$kBaseUrl/deletePublicSender?key=${widget.sessionKey}&session=$sessionName'),
      ).timeout(const Duration(seconds: 10));
      final d = jsonDecode(res.body);
      if (d['valid'] == true) {
  ...
  Future<void> _togglePublic(String sessionName, bool makePublic) async {
    try {
      final res = await http.get(
        Uri.parse('$kBaseUrl/setSenderPublic?key=${widget.sessionKey}&session=$sessionName&public=$makePublic'),
      ).timeout(const Duration(seconds: 10));
      final d = jsonDecode(res.body);
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        elevation: 0,
        title: Row(children: [
          Icon(Icons.router_rounded, color: _C.accentL, size: 18),
          const SizedBox(width: 8),
          Text('Manage Sender', style: TextStyle(color: _C.textP, fontSize: 15, fontWeight: FontWeight.bold)),
        ]),
      ...
      floatingActionButton: FloatingActionButton.extended(
        ...
        icon: _addingPrivate
            ? SizedBox(width: 18, height: 18, child: CircularProgressIndicator(color: _C.white, strokeWidth: 2))
            : Icon(Icons.add_rounded, color: _C.white),
        label: Text('Tambah Sender', style: TextStyle(color: _C.white, fontWeight: FontWeight.bold)),
    ...
    return Column(children: [
      ...
      if (_pairingCode != null)
        Container(
          ...
          child: Column(children: [
            Text('PAIRING CODE', style: TextStyle(color: _C.accentL, fontSize: 11, letterSpacing: 2, fontWeight: FontWeight.bold)),
            const SizedBox(height: 10),
            // Kode + tombol salin
            Row(
              mainAxisAlignment: MainAxisAlignment.center,
              children: [
                Text(_pairingCode!, style: TextStyle(color: _C.white, fontSize: 28, fontWeight: FontWeight.w900, letterSpacing: 6)),
                ...
                GestureDetector(
                  ...
                  child: Container(
                    ...
                    decoration: BoxDecoration(
                      color: _C.accentL.withOpacity(0.15),
                      borderRadius: BorderRadius.circular(8),
                      border: Border.all(color: _C.accentL.withOpacity(0.4)),
                    ),
                    child: Icon(Icons.copy_rounded, color: _C.accentL, size: 18),
                  ),
                ),
              ],
            ),
            const SizedBox(height: 8),
            Text('Masukkan kode ini di WhatsApp → Perangkat Tertaut → Tautkan dengan nomor telepon', style: TextStyle(color: _C.textS, fontSize: 11), textAlign: TextAlign.center),
            const SizedBox(height: 10),
            GestureDetector(
              onTap: () => setState(() => _pairingCode = null),
              child: Text('Tutup', style: TextStyle(color: _C.red, fontSize: 12)),
            ),
          ]),
        ),
      ...
      Padding(padding: const EdgeInsets.fromLTRB(14, 10, 14, 4), child: Row(children: [
        ...
        GestureDetector(
          ...
          child: Icon(Icons.refresh_rounded, color: _C.accentL, size: 18)),
      ])),
          ...
          child: Column(mainAxisSize: MainAxisSize.min, children: [
            Container(
              ...
              decoration: BoxDecoration(
                ...
                border: Border.all(color: Color(0x4CF06292)),
              ),
    ...
    return Container(
      ...
      child: Row(children: [
        ...
        Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Text(name, style: TextStyle(color: _C.textP, fontWeight: FontWeight.bold, fontSize: 13),
          ...
          Row(children: [
            ...
            if (isPublic && owner != null) ...[
              const SizedBox(width: 8),
              Text('by $owner', style: TextStyle(color: _C.textM, fontSize: 10)),
            ],
          ]),
        ])),
        // Actions
  ...
  void _confirmDelete(String name, bool isPublic) {
    showDialog(context: context, builder: (_) => AlertDialog(
      ...
      title: Text('Hapus Sender?', style: TextStyle(color: _C.textP)),
      content: Text('$name akan dihapus.', style: TextStyle(color: _C.textS)),
      actions: [
        TextButton(onPressed: () => Navigator.pop(context), child: Text('Batal', style: TextStyle(color: _C.textS))),
        TextButton(
          onPressed: () {
          ...
          child: Text('Hapus', style: TextStyle(color: _C.red, fontWeight: FontWeight.bold))),
      ],
    ));
  }
  ...
  void _showAddDialog() {
    ...
    showDialog(context: context, builder: (_) => AlertDialog(
      backgroundColor: _C.s1,
      shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(14), side: BorderSide(color: _C.border)),
      title: Text('Tambah Sender', style: TextStyle(color: _C.textP, fontWeight: FontWeight.bold)),
      content: Column(mainAxisSize: MainAxisSize.min, children: [
        Text('Masukkan nomor WA (dengan kode negara, tanpa +)', style: TextStyle(color: _C.textS, fontSize: 12)),
        const SizedBox(height: 12),
        TextField(
          controller: _numCtrl,
          keyboardType: TextInputType.phone,
          style: TextStyle(color: _C.textP),
      ...
      actions: [
        ...
        ElevatedButton(
          ...
          child: Text('Generate Pairing', style: TextStyle(color: _C.white, fontWeight: FontWeight.bold))),
      ],
    ));
  }
  ...
  Widget _emptyWidget(String msg, IconData icon) => Center(child: Column(mainAxisSize: MainAxisSize.min, children: [
    ...
    Text(msg, style: TextStyle(color: _C.textS, fontSize: 13), textAlign: TextAlign.center),
  ]));

  Widget _errorWidget(String err, VoidCallback retry) => Center(child: Column(mainAxisSize: MainAxisSize.min, children: [
    Icon(Icons.error_outline_rounded, color: _C.red, size: 40),
    const SizedBox(height: 10),
    Text(err, style: TextStyle(color: _C.textS, fontSize: 12), textAlign: TextAlign.center),
    const SizedBox(height: 14),
    GestureDetector(onTap: retry, child: Container(
      ...
      child: Text('Coba Lagi', style: TextStyle(color: _C.accentL, fontWeight: FontWeight.bold)))),
  ]));
}
