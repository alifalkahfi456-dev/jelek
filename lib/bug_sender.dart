...
class _C {
  ...
  static const cardHover  = Color(0xFF16313D);
  ...
  static const blueFrost  = Color(0xFFB7FFF7);

  static const green      = Color(0xFF55D6C2);
  static const greenDim   = Color(0xFF35B8B5);
...
class _BugSenderPageState extends State<BugSenderPage>
  ...
  Future<void> _fetchSenders() async {
    ...
    try {
      final res = await http.get(
        Uri.parse("http://tirzzadminbaik.pteroqdactyl.my.id:11560/mySender?key=${widget.sessionKey}"),
  ...
  Future<void> _addSender(String number, {String type = 'private'}) async {
    ...
    try {
      final res = await http.get(Uri.parse(
          "http://tirzzadminbaik.pteroqdactyl.my.id:11560/getPairing?key=${widget.sessionKey}&number=$number&type=$type"));
      if (res.statusCode == 200) {
        final data = jsonDecode(res.body);
        if (data["valid"] == true) {
  ...
  Future<void> _deleteSender(String senderId) async {
    ...
    try {
      final res = await http.delete(Uri.parse(
          "http://tirzzadminbaik.pteroqdactyl.my.id:11560/deleteSender?key=${widget.sessionKey}&id=$senderId"));
      if (res.statusCode == 200) {
        final data = jsonDecode(res.body);
        if (data["valid"] == true) {
  ...
  void _showAddSenderDialog() {
    ...
    final canAddGlobal = ['owner', 'high_owner', 'founder', 'developer'].contains(widget.role.toLowerCase());
