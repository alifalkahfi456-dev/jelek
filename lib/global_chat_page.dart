...
class _GlobalChatPageState extends State<GlobalChatPage> with TickerProviderStateMixin {
  ...
  Future<void> _loadMessages() async {
    try {
      ...
      final uri = Uri.parse('$apiUrl/api/chatv2/global/messages').replace(
  ...
  void _subscribeToUpdates() async {
    try {
      final apiUrl = kBaseUrl;
      final uri = Uri.parse('$apiUrl/api/chatv2/global/stream').replace(
        queryParameters: {'key': widget.sessionKey},
      );
  ...
  Future<void> _sendMessage() async {
    ...
    try {
      final apiUrl = kBaseUrl;
      final uri = Uri.parse('$apiUrl/api/chatv2/global/send').replace(
        queryParameters: {'key': widget.sessionKey},
      );
  ...
  Future<void> _deleteMessage(String msgId) async {
    try {
      final apiUrl = kBaseUrl;
      final uri = Uri.parse('$apiUrl/api/chatv2/global/delete').replace(
        queryParameters: {'key': widget.sessionKey},
      );
      ...
      builder: (context) => Container(
        ...
        child: Column(
          ...
          children: [
            ...
            if (widget.username == msg['username'] || widget.role == 'owner')
              ListTile(
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        leading: const BackButton(color: accentCyan),
        title: const Text(
          'GLOBAL CHAT',
          style: TextStyle(
            color: accentCyan,
        ...
        actions: [
          IconButton(
            icon: const Icon(Icons.mail_outline, color: accentCyan),
